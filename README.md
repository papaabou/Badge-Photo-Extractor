...
9. **Générateur de badges imprimables** — le bouton « 🪪 Générer les badges (PDF) » ouvre un panneau permettant de choisir un format de carte (CR80 86×54 mm ou badge conférence 90×120 mm), un sous-titre commun, un logo d'entreprise et un QR code optionnel, puis génère une planche PDF A4 (via [jsPDF](https://github.com/parallax/jsPDF)) avec autant de badges que possible par page et des repères de coupe aux 4 coins. Le QR code encode une fiche de contact (vCard : nom + sous-titre) plutôt qu'un simple texte — au scan, la personne peut être ajoutée directement aux contacts du téléphone. Il est généré localement avec [qrcodejs](https://github.com/davidshimjs/qrcodejs) — aucune donnée n'y transite. Les accents sont retirés du contenu du QR pour garantir une lecture fiable par tous les lecteurs (byte-mode QR sans indicateur UTF-8).
...
Aucune bibliothèque n'est installée localement : PDF.js, JSZip, jsPDF, qrcodejs et MediaPipe Tasks Vision sont chargés depuis un CDN (cdnjs / jsDelivr) directement dans `index.html`.
